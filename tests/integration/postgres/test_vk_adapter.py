...
def make_processor(
...
) -> tuple[VkUpdateProcessor, RecordingVkSender]:
    ...
    return VkUpdateProcessor(service, renderer, uow_factory, delivery), sender


async def test_lora_complete_flow_persists_through_vk_adapter(
    uow_factory: PostgresUnitOfWorkFactory,
) -> None:
    ...
    await processor.process_message_event(
        callback_event(pack_payload(SelectGameCallback("lora_dein")), event_id="2")
    )
    await processor.process_message(text_message("1,3", message_id=3))
    await processor.process_message(text_message("1,2", message_id=4))
    await processor.process_message(text_message("6", message_id=5))
...
async def test_margo_q9_persists_audio_and_scheduled_row(
    ...
    sender.calls.clear()
    await processor.process_message(text_message("31,33", message_id=2))
